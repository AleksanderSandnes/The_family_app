...
import com.example.mainactivity.adapter.CalendarPageAdapter;
...
public class CalendarPageFragment extends Fragment {
    ...
    private ArrayList<CalendarPageModel> aktiviteter;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        nyAktivitet.setOnClickListener(Navigation.createNavigateOnClickListener(R.id.action_calendarFragment2_to_calendarAddFragment));
    }

    // Sorterer calendar og oppdaterer recyclerView
    private void setUpRecyclerView() {
        RecyclerView calendarRecyclerView = getView().findViewById(R.id.activityOnDay);

        Comparator<CalendarPageModel> byTime = new Comparator<CalendarPageModel>() {
            @Override
            public int compare(CalendarPageModel o1, CalendarPageModel o2) {
        ...
        calendarRecyclerView.setAdapter(new CalendarPageAdapter(getContext(), aktiviteter));
    ...
    private void setActivitiesAndIds() {
        ...
        ArrayList<CalendarPageModel> aktiviteter = new ArrayList<>();
            ...
            if (familieID != null) {
                if (familieID.equals(sharedPreferences.getString(User.FAMILIE, null))){
                    if (fullDateFrom.getDate() == fullDateSelected.getDate() && fullDateFrom.getMonth() == fullDateSelected.getMonth()) {
                        String birthdayFor = name + " har birthday i dag!";
                        CalendarPageModel calendarPageModel = new CalendarPageModel(dato, null, "00:01", null, null, birthdayFor, 0, 0, true);
                        aktiviteter.add(calendarPageModel);
                    }
                }
            }
        }
        ...
        while (data.moveToNext()) {
            ...
            // Dersom man har samme familie-ID vil den bli lagt til i list
            if (userFamily.equals(sharedPreferences.getString(User.FAMILIE, null))){
                if (fullDateTo != null) { // Dersom det er en til dato og aktiviteten er lik eller mellom datoene blir den lagt med
                    if ((fullDateFrom.before(fullDateSelected) || fullDateFrom.equals(fullDateSelected)) && (fullDateTo.after(fullDateSelected) || fullDateTo.equals(fullDateSelected))) {
                        CalendarPageModel calendarPageModel = new CalendarPageModel(dateFrom, dateTo, timeFrom, timeTo, userName, theActivity, userID, activityID, false);
                        aktiviteter.add(calendarPageModel);
                    }
